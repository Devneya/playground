...
const content = (id: string, title = id): PlaygroundNode => ({
  ...
  data: { kind: "content", title, text: `${title} text`, origin: "imported" },
...
describe("graph invariants", () => {
  it("starts with a valid prompt flow with no inputs yet", () => {
    ...
    expect(prompt.data.title).toBe("Prompt 1");
  ...
  it("rejects wrong endpoint types and duplicate inputs", () => {
    ...
    const flow = workspace.flows[0]!;
    const prompt = flow.nodes.find((node) => node.data.kind === "prompt")!;
    expect(canAddInputConnection(flow, prompt.id, prompt.id)).toEqual({ allowed: false, reason: "Connect Content to Prompt only." });
    ...
    expect(canAddInputConnection(connected, "c1", prompt.id)).toEqual({ allowed: false, reason: "That Content node is already connected." });
    const extra = { ...connected, nodes: [...connected.nodes, content("c2", "Other")] };
    expect(canAddInputConnection(extra, "c2", prompt.id)).toEqual({ allowed: false, reason: "A Prompt node can have only one input." });
  });
  ...
  it("rejects a cycle and unavailable generated result as an input", () => {
    ...
    const result: PlaygroundNode = {
      id: "r1",
      position: { x: 500, y: 120 },
      data: { kind: "content", title: "model", text: "out", origin: "generated", modelId: "model", batchId: "b", executionId: "e" },
      createdAt: systemClock.now().toISOString(),
      updatedAt: systemClock.now().toISOString(),
    };
    const produced = {
      ...
      batches: [{ id: "b", promptNodeId: prompt.id, startedAt: systemClock.now().toISOString(), promptFormatVersion: 1 as const, instruction: "", inputs: [], executions: [{ id: "e", modelId: "model", status: "success" as const, startedAt: systemClock.now().toISOString(), outputNodeId: result.id }] }],
    };
