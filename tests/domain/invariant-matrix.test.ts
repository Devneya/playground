...
const manual = (nodeId: string, text = "text"): PlaygroundNode => ({ id: nodeId, position: { x: 50, y: 50 }, data: { kind: "content", origin: "imported", title: nodeId, text }, createdAt: clock.now().toISOString(), updatedAt: clock.now().toISOString() });
...
describe("workspace invariant branch matrix", () => {
  ...
  it("rejects node, edge, model, and provenance violations", () => {
    ...
    const invalidPrompt = { ...prompt, data: { kind: "prompt" as const, title: "p", prompt: "x".repeat(LIMITS.maxTextBytes + 1), modelIds: ["a", "a", "b", "c", "d"] } };
    const invalidContent = { ...manual("bad", "x"), position: { x: Number.NaN, y: 0 }, data: { kind: "content" as const, origin: "imported" as const, title: "x".repeat(LIMITS.maxNodeTitleCodePoints + 1), text: "x".repeat(LIMITS.maxTextBytes + 1) } };
    const generated = { ...manual("result"), data: { kind: "content" as const, origin: "generated" as const, title: "wrong-model", text: "result", modelId: "wrong-model", batchId: "missing-batch", executionId: "missing-execution" } };
    ...
    expect(errors).toEqual(expect.arrayContaining([
      ...
      expect.stringContaining("prompt is too large"),
  ...
  it("rejects unavailable generated inputs, duplicate/order violations, bad batches, and cycles", () => {
    ...
    const generated = { ...manual("result"), data: { kind: "content" as const, origin: "generated" as const, title: "model-a", text: "", modelId: "model-a", batchId: "batch", executionId: "execution" } };
    const batch: ExecutionBatch = { id: "batch", promptNodeId: prompt.id, startedAt: clock.now().toISOString(), promptFormatVersion: 1, instruction: "", inputs: [], executions: [{ id: "execution", modelId: "model-a", status: "pending", startedAt: clock.now().toISOString(), outputNodeId: generated.id }] };
    const edges: PlaygroundEdge[] = [
    ...
    const invalidBatch = { ...batch, promptNodeId: "missing-prompt", executions: Array.from({ length: 5 }, (_, index) => ({ ...batch.executions[0]!, id: `execution-${index}`, modelId: index === 4 ? "model-a" : `model-${index}`, outputNodeId: "wrong-output" })) };
  ...
  it("covers connection rejection reasons and successful reconnection", () => {
    ...
    const flow = workspace.flows[0]!;
    const prompt = flow.nodes[0]!;
    expect(canAddInputConnection(flow, prompt.id, prompt.id)).toMatchObject({ allowed: false, reason: "Connect Content to Prompt only." });
    ...
    expect(canAddInputConnection({ ...connected, nodes: [...connected.nodes, extra] }, extra.id, prompt.id)).toMatchObject({ allowed: false, reason: "A Prompt node can have only one input." });
    expect(canAddInputConnection(connected, first.id, prompt.id)).toMatchObject({ allowed: false, reason: "That Content node is already connected." });
  ...
  it("covers malformed duplication references and optional provenance fields", () => {
    ...
    const generated: PlaygroundNode = {
      ...
      data: { kind: "content", origin: "generated", title: "model", text: "", modelId: "model", batchId: "missing-batch", executionId: "missing-execution" },
    ...
    expect(duplicate.nodes.find((node) => node.data.kind === "content" && node.data.origin === "generated")?.data).toMatchObject({ batchId: "missing-batch", executionId: "missing-execution" });
    expect(duplicate.edges.find((edge) => edge.source === "missing-source")).toMatchObject({ source: "missing-source", target: "missing-target" });
    expect(duplicate.batches[0]).toMatchObject({ promptNodeId: "missing-prompt", inputs: [{ nodeId: "missing-input" }] });
  ...
  it("covers graph traversal, non-content snapshots, and structural capacity errors", () => {
    ...
    const generated = { ...manual("generated-success"), data: { kind: "content" as const, origin: "generated" as const, title: "model", text: "", modelId: "model", batchId: "batch-success", executionId: "execution-success" } };
    const generatedFlow = { ...flow, nodes: [...flow.nodes, generated], edges: [...flow.edges, { id: "generated-success-input", kind: "input" as const, source: generated.id, target: prompt.id, order: 1 }, { id: "wrong-result", kind: "result" as const, source: prompt.id, target: prompt.id }], batches: [{ id: "batch-success", promptNodeId: prompt.id, startedAt: clock.now().toISOString(), promptFormatVersion: 1 as const, instruction: "", inputs: [], executions: [{ id: "execution-success", modelId: "model", status: "success" as const, startedAt: clock.now().toISOString() }] }] };
  ...
  it("covers reducer no-op paths, single-flow reset, and history bounds", () => {
    ...
    expect(() => parseWorkspaceExport({ format: "devneya-flow-v2", exportedAt: clock.now().toISOString(), workspace: large })).toThrow("too large");
  });
