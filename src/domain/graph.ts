...
import { isContentNode, isGeneratedContentNode, isPromptNode } from "./types";

export const getNode = (flow: FlowDocument, nodeId: string) => flow.nodes.find((node) => node.id === nodeId);

export const getOrderedInputEdges = (flow: FlowDocument, promptNodeId: string): InputEdge[] =>
  flow.edges.filter((edge): edge is InputEdge => edge.kind === "input" && edge.target === promptNodeId).sort((a, b) => a.order - b.order);

export const getInputSnapshots = (flow: FlowDocument, promptNodeId: string): InputSnapshot[] =>
  getOrderedInputEdges(flow, promptNodeId).flatMap((edge) => {
    const node = getNode(flow, edge.source);
    return isContentNode(node) ? [{ nodeId: node.id, title: node.data.title, text: node.data.text }] : [];
  });
...
export const canAddInputConnection = (flow: FlowDocument, sourceContentId: string, targetPromptId: string, ignoredEdgeId?: string): { allowed: true } | { allowed: false; reason: string } => {
  const source = getNode(flow, sourceContentId);
  const target = getNode(flow, targetPromptId);
  if (!isContentNode(source) || !isPromptNode(target)) return { allowed: false, reason: "Connect Content to Prompt only." };
  if (source.id === target.id) return { allowed: false, reason: "A node cannot connect to itself." };
  if (source.data.origin === "generated") {
  ...
  if (getOrderedInputEdges(flow, target.id).some((edge) => edge.id !== ignoredEdgeId && edge.source === source.id)) return { allowed: false, reason: "That Content node is already connected." };
  if (getOrderedInputEdges(flow, target.id).filter((edge) => edge.id !== ignoredEdgeId).length >= LIMITS.maxInputsPerGeneration) return { allowed: false, reason: "A Prompt node can have only one input." };
...
export const validateWorkspaceInvariants = (workspace: WorkspaceDocument): string[] => {
  const errors: string[] = [];
  if (workspace.schemaVersion !== 2) errors.push("Unsupported workspace schema version.");
  ...
  for (const flow of workspace.flows) {
    ...
    for (const node of flow.nodes) {
      ...
      if (node.data.kind === "prompt") {
        ...
        if (utf8ByteLength(node.data.prompt) > LIMITS.maxTextBytes) errors.push(`Node ${node.id} prompt is too large.`);
    ...
    for (const edge of flow.edges) {
      ...
      if (edge.kind === "input") {
        if (!isContentNode(source) || !isPromptNode(target)) errors.push(`Input edge ${edge.id} has invalid endpoints.`);
        if (isGeneratedContentNode(source)) {
          const execution = flow.batches.flatMap((batch) => batch.executions).find((item) => item.id === source.data.executionId);
          if (!execution || execution.status !== "success") errors.push(`Input edge ${edge.id} uses an unavailable result.`);
        }
      } else if (!isPromptNode(source) || !isGeneratedContentNode(target)) errors.push(`Result edge ${edge.id} has invalid endpoints.`);
    }
    for (const prompt of flow.nodes.filter(isPromptNode)) {
      const inputEdges = getOrderedInputEdges(flow, prompt.id);
      if (inputEdges.some((edge, index) => edge.order !== index)) errors.push(`Prompt ${prompt.id} input order is not contiguous.`);
      if (new Set(inputEdges.map((edge) => edge.source)).size !== inputEdges.length) errors.push(`Prompt ${prompt.id} has duplicate inputs.`);
    }
    for (const node of flow.nodes.filter(isGeneratedContentNode)) {
      const execution = flow.batches.flatMap((batch) => batch.executions).find((item) => item.id === node.data.executionId);
      if (!execution || execution.outputNodeId !== node.id || execution.modelId !== (node.data.modelId ?? node.data.title)) errors.push(`Generated node ${node.id} has an invalid provenance reference.`);
    }
    for (const batch of flow.batches) {
      if (!nodes.has(batch.promptNodeId)) errors.push(`Batch ${batch.id} has a missing source prompt node.`);
      ...
      for (const execution of batch.executions) {
        if (execution.outputNodeId) {
          ...
          if (!isGeneratedContentNode(output) || output.data.executionId !== execution.id) errors.push(`Execution ${execution.id} points to the wrong output.`);
        }
      }
    }
