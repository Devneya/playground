...
import type { PromptData } from "../../domain/types";
import { useWorkspace } from "../workspace/useWorkspace";
import { ModelPicker } from "./ModelPicker";

type PromptFlowNode = Node<PromptData, "prompt">;
...
export const PromptNode = ({ id, data }: NodeProps<PromptFlowNode>) => {
  ...
  const runningBatch = useMemo(() => activeFlow.batches.find((batch) => batch.promptNodeId === id && batch.executions.some((execution) => execution.status === "pending")), [activeFlow.batches, id]);
  ...
  return <article className="flow-node prompt-node">
    <Handle type="target" position={Position.Left} id="prompt-input" />
    <Handle type="source" position={Position.Right} id="prompt-output" isConnectable={false} />
    ...
    <label className="node-field"><textarea className="node-textarea instruction-textarea" aria-label={`${data.title} prompt`} value={data.prompt} onChange={(event) => dispatch({ type: "node/edit-prompt", flowId: activeFlow.id, nodeId: id, prompt: event.target.value })} placeholder="Write a prompt, then run it…" /></label>
    <div className="input-order" aria-label="Prompt inputs">
      <div className="field-label visually-hidden">Inputs <span className="muted">({inputs.length})</span></div>
      {inputs.length === 0 ? <span className="muted">Drag from a content node’s right dot onto this card.</span> : inputs.map((edge, index) => {
        const source = activeFlow.nodes.find((node) => node.id === edge.source);
        return <div className="input-row" key={edge.id}><span>{source?.data.title ?? "Content"}</span><button type="button" className="icon-button" aria-label={`Remove input ${index + 1}`} onClick={() => dispatch({ type: "input/remove", flowId: activeFlow.id, edgeId: edge.id })}>×</button></div>;
      })}
    </div>
    ...
    <div className="node-run-row"><button type="button" className={runningBatchId ? "send-button running" : "send-button"} aria-label={runningBatchId ? "Cancel run" : "Run prompt"} title={runningBatchId ? "Cancel run" : "Run prompt"} onClick={runningBatchId ? () => cancelRun(runningBatchId) : run} disabled={!runningBatchId && (!virtualKey || data.modelIds.length === 0)}>{runningBatchId ? "■" : "➤"}</button></div>
  </article>;
};
