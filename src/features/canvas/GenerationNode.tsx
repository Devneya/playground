...
export const GenerationNode = ({ id, data }: NodeProps<GenerationFlowNode>) => {
  ...
  return <article className={`flow-node generation-node ${dropValid ? "drop-target" : ""}`}>
    ...
    <div className="context-order" aria-label="Generation context">
      <div className="field-label">Context <span className="muted">({inputs.length})</span></div>
      {inputs.length === 0 ? <span className="muted">Drag a Text node's right dot onto this card to add it as context.</span> : inputs.map((edge, index) => {
